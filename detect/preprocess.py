...
def resize_image(im, max_img_size=cfg.max_train_img_size):
    ...
    # print("image's width is:", im.width)
    # print("-"*10)
    im_width = np.minimum(im.width, max_img_size)  # np.minimum(x1，x2)函数就是对比x1和x2的大小（若是数组则一个一个位置比较）求值
    # print("im_width is:", im_width)
    # 下面语句利用已知的width进行缩放图片
    ...
    # 下面语句将图片的size控制在几个数字中（利用32）
    d_wight = o_width - (o_width % 32)
    d_height = o_height - (o_height % 32)
    return d_wight, d_height    # 返回真正符合要求的宽高
