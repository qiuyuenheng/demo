...
# recognize api
class PytorchOcr():
    def __init__(self, model_path=r'F:\ocr_demo1\ch_recognize\models/CRNN-1010.pth'):
        alphabet_unicode = config.alphabet_v2
        # print("alphabet_v2 is:", alphabet_unicode)
        self.alphabet = ''.join([chr(uni) for uni in alphabet_unicode])
        # print(len(self.alphabet)) = 5834
        self.nclass = len(self.alphabet) + 1
        self.model = CRNN(config.imgH, 1, self.nclass, 256)     # 输入图片高固定为32
        self.cuda = False
    ...
    def recognize(self, img):  # img是ctpn检测出来的文本框
        h, w = img.shape[:2]
